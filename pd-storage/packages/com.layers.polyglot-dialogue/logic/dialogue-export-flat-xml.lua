--------------------------------------------------------------------------------
--  Logic-export..... : dialogue-export-flat-xml
--  Author........... :
--  Description...... :
--------------------------------------------------------------------------------
...
local listener = {
    ...
    Template = "dialogue-export-xml",
    Call = "com.layers.polyglot-visualnode.lib.dialogue-export-flat-xml"
}
...
traverse[listener["Call"] .. ".before"] = function ()
    log.message( "dialogue-export-flat-xml" .. ".before" )
...
traverse[listener["Call"] .. ".dialog"] = function (data, tNode)
    log.message ( "dialogue-export-flat-xml.dialog" )
    if(tNode)
    then
        io.write( "\n" )
...
traverse[listener["Call"] .. ".actor"] = function (data, tNode)
    log.message ( "dialogue-export-flat-xml.actor" )
    if(tNode)
    then
        io.write( "\n" )
        for kN,vN in pairs(tNode) do
            io.write('\n<item type="actor" id="a(', indexedId ,')" field="', kN , '" value="', vN ,'" />')
        end
        indexedId = indexedId + 1
    end
end
...
traverse[listener["Call"] .. ".after"] = function ()
    log.message( "dialogue-export-flat-xml" .. ".after" )
    io.write( "\n\n</items>" )
    io.close( file )
...
function  main()
--------------------------------------------------------------------------------
    log.message( "Running: " .. "dialogue-export-flat-xml" )
